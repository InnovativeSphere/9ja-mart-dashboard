...
import { X, Store, User, Mail, Package, DollarSign, Star, Calendar } from "lucide-react";
...
export default function SellerViewModal({ seller, onClose }: Props) {
  useEffect(() => {
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-card" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-grid">
          <Detail icon={Store} label="Store" value={seller.store} />
          <Detail icon={Mail} label="Email" value={seller.email} />
          <Detail icon={Package} label="Products" value={`${seller.products}`} />
          <Detail icon={DollarSign} label="Sales" value={seller.sales} />
          <Detail icon={Star} label="Rating" value={seller.rating > 0 ? seller.rating.toFixed(1) : "N/A"} />
          <Detail icon={Calendar} label="Joined" value={seller.joined} />
        </div>
      </div>
      <style jsx>{`
