...
import { X, Save, User, Mail, MapPin, BarChart3 } from "lucide-react";
...
export default function BuyerFormModal({ buyer, onClose, onSave }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onSave({ id: buyer?.id, name, email, location, status });
    onClose();
  };
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-card" onClick={(e) => e.stopPropagation()}>
        <div className="modal-header">
          <h2 className="text-xl font-bold">{isEdit ? "Edit Buyer" : "New Buyer"}</h2>
        ...
        <form onSubmit={handleSubmit} className="form-body">
          ...
          <div className="form-row">
            <div className="field-group">
              <label className="field-label"><MapPin size={15} /> Location</label>
              <input
                type="text"
                value={location}
          ...
          <button type="submit" className="submit-btn">
            <Save size={16} /> {isEdit ? "Update Buyer" : "Create Buyer"}
          </button>
        </form>
      </div>
